    ...
    container.innerHTML = `
      <div class="space-y-8">
        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div>
            <h1 class="text-2xl lg:text-3xl font-bold">${t('dashboard.welcome', 'Welcome back,')} ${clientName}!</h1>
            <p class="text-gray-400 mt-1">${t('dashboard.subtitle', "Here's what's happening with your content")}</p>
          </div>
          <div class="flex gap-3">
            ...
            <a href="#/batches/new" class="flex items-center gap-2 px-4 py-2 bg-slate-700 hover:bg-slate-600 rounded-lg transition-colors">
              ...
              ${t('dashboard.newBatch', 'New Batch')}
            </a>
          </div>
        </div>
          ...
          <div class="bg-slate-800/50 rounded-xl p-4 border border-slate-700">
            <div class="flex items-center gap-3">
              ...
              <div>
                ...
                <p class="text-sm text-gray-400">${t('dashboard.scheduled', 'Scheduled')}</p>
              </div>
            </div>
          </div>
          ...
          <div class="bg-slate-800/50 rounded-xl p-4 border border-slate-700">
            <div class="flex items-center gap-3">
              ...
              <div>
                ...
                <p class="text-sm text-gray-400">${t('dashboard.posted', 'Posted')}</p>
              </div>
            </div>
          </div>
          ...
          <div class="bg-slate-800/50 rounded-xl p-4 border border-slate-700">
            <div class="flex items-center gap-3">
              ...
              <div>
                ...
                <p class="text-sm text-gray-400">${t('dashboard.pending', 'Pending')}</p>
              </div>
            </div>
          </div>
          ...
          <div class="bg-slate-800/50 rounded-xl p-4 border border-slate-700">
            <div class="flex items-center gap-3">
              ...
              <div>
                ...
                <p class="text-sm text-gray-400">${t('dashboard.failed', 'Failed')}</p>
              </div>
            </div>
          </div>
        </div>
          ...
          <div class="bg-slate-800/50 rounded-xl border border-slate-700">
            <div class="p-4 border-b border-slate-700 flex items-center justify-between">
              <h2 class="font-semibold flex items-center gap-2">
                ...
                ${t('dashboard.upcomingPosts', 'Upcoming Posts')}
              </h2>
              <a href="#/posts" class="text-sm text-brandBlue hover:underline">${t('dashboard.viewAll', 'View all')}</a>
            </div>
            <div class="divide-y divide-slate-700">
              ${upcomingPosts.length === 0 ? `
                <div class="p-8 text-center text-gray-400">
                  <i data-lucide="inbox" class="w-12 h-12 mx-auto mb-3 opacity-50"></i>
                  <p>${t('dashboard.noScheduled', 'No scheduled posts')}</p>
                  <a href="#/posts/new" class="text-brandBlue hover:underline text-sm">${t('dashboard.createFirst', 'Create your first post')}</a>
                </div>
                  ...
                  <div class="flex items-start gap-3">
                    ...
                    <div class="flex-1 min-w-0">
                      <div class="flex items-center gap-2 mb-1">
                        ...
                        <span class="text-sm font-medium">${post.subject || t('dashboard.untitled', 'Untitled')}</span>
                      </div>
                  ...
                  <div class="flex items-center gap-3">
                    ...
                    <div class="flex-1">
                      <p class="text-sm">
                        ...
                        <span class="text-gray-400">${t('dashboard.was', 'was')} ${post.status}</span>
                      </p>
        ...
        <div class="grid sm:grid-cols-3 gap-4">
          <a href="#/media" class="bg-slate-800/50 rounded-xl p-6 border border-slate-700 hover:border-brandBlue transition-colors group">
            ...
            <h3 class="font-semibold mb-1">${t('dashboard.mediaLibrary', 'Media Library')}</h3>
            <p class="text-sm text-gray-400">${t('dashboard.uploadManage', 'Upload and manage your images')}</p>
          </a>
          ...
          <a href="#/schedule" class="bg-slate-800/50 rounded-xl p-6 border border-slate-700 hover:border-brandBlue transition-colors group">
            ...
            <h3 class="font-semibold mb-1">${t('dashboard.scheduleSettings', 'Schedule Settings')}</h3>
            <p class="text-sm text-gray-400">${t('dashboard.configureTimes', 'Configure posting times')}</p>
          </a>
          ...
          <a href="#/batches/new" class="bg-slate-800/50 rounded-xl p-6 border border-slate-700 hover:border-brandBlue transition-colors group">
            ...
            <h3 class="font-semibold mb-1">${t('dashboard.bulkUpload', 'Bulk Upload')}</h3>
            <p class="text-sm text-gray-400">${t('dashboard.scheduleMultiple', 'Schedule multiple posts at once')}</p>
          </a>
        </div>
      </div>
    `;
