    ...
    container.innerHTML = `
      <div class="space-y-6">
        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div>
            <h1 class="text-2xl lg:text-3xl font-bold">Content Calendar</h1>
            <p class="text-gray-400 mt-1">View and manage your posting schedule</p>
          </div>
          <div class="flex items-center gap-3">
            <div class="flex items-center bg-slate-800 rounded-lg p-1">
              <button id="view-month" class="px-3 py-1.5 rounded-md text-sm font-medium ${this.view === 'month' ? 'bg-brandBlue' : 'hover:bg-slate-700'}">Month</button>
              <button id="view-week" class="px-3 py-1.5 rounded-md text-sm font-medium ${this.view === 'week' ? 'bg-brandBlue' : 'hover:bg-slate-700'}">Week</button>
            </div>
        ...
        <div class="flex items-center justify-between bg-slate-800/50 rounded-xl p-4 border border-slate-700">
          ...
          <div class="flex items-center gap-3">
            ...
            <button id="today-btn" class="px-3 py-1 text-sm bg-slate-700 hover:bg-slate-600 rounded-lg transition-colors">Today</button>
          </div>
        ...
        <div class="flex flex-wrap items-center gap-4 text-sm">
          <span class="flex items-center gap-2">
            <span class="w-3 h-3 rounded-full bg-green-500"></span> Posted
          </span>
          <span class="flex items-center gap-2">
            <span class="w-3 h-3 rounded-full bg-brandBlue"></span> Scheduled
          </span>
          <span class="flex items-center gap-2">
    ...
    try {
      ...
      container.innerHTML = `
        <div class="p-8 text-center text-red-400">
          ...
          <button onclick="Calendar.loadCalendar()" class="mt-4 text-brandBlue hover:underline">Try again</button>
        </div>
      `;
      lucide.createIcons();
    }
  ...
  renderMonthView(startDate, endDate) {
    ...
    container.innerHTML = `
      ...
      <div class="grid grid-cols-7 border-b border-slate-700">
        ${dayNames.map(d => `
          <div class="p-3 text-center text-sm font-medium text-gray-400 ${['Sun', 'Sat'].includes(d) ? 'bg-slate-900/30' : ''}">${d}</div>
        `).join('')}
      </div>
              ...
              <div class="space-y-1">
                ${dayPosts.slice(0, 3).map(post => `
                  <div class="flex items-center gap-1 text-xs p-1 rounded ${this.getStatusColor(post.status)}/20 truncate">
                    ...
                    <span class="truncate">${post.subject || 'Post'}</span>
                  </div>
                `).join('')}
                ${dayPosts.length > 3 ? `
                  <div class="text-xs text-gray-500 pl-2">+${dayPosts.length - 3} more</div>
                ` : ''}
              </div>
            </div>
          `;
              ...
              <div class="flex-1 p-3 min-h-24">
                ${dayPosts.length === 0 ? `
                  <div class="h-full flex items-center justify-center">
                    ...
                    >
                      <i data-lucide="plus" class="w-4 h-4 inline"></i> Add post
                    </button>
                  </div>
                ` : `
                  <div class="space-y-2">
                      ...
                      <a href="#/posts/${post.post_id}" class="flex items-center gap-3 p-2 rounded-lg hover:bg-slate-700/50 transition-colors group">
                        ...
                        <div class="flex-1 min-w-0">
                          <div class="flex items-center gap-2">
                            ...
                            <span class="font-medium truncate group-hover:text-brandBlue">${post.subject || 'Untitled'}</span>
                          </div>
                          <p class="text-xs text-gray-400 truncate">${post.caption?.slice(0, 40) || 'No caption'}...</p>
                        </div>
                    ...
                    >
                      <i data-lucide="plus" class="w-3 h-3 inline"></i> Add more
                    </button>
                  </div>
                `}
              </div>
          ...
          ${dayPosts.map(post => `
            <a href="#/posts/${post.post_id}" class="flex items-center gap-3 p-3 rounded-lg bg-slate-700/50 hover:bg-slate-700 transition-colors">
              ...
              <div class="flex-1 min-w-0">
                ...
                <p class="text-xs text-gray-400 truncate">${post.caption?.slice(0, 50) || 'No caption'}</p>
              </div>
